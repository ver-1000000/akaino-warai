...
class Entity {
    constructor(uniqQuery) {
        ...
        this.name = uniqQuery in QUERY_NAME_MAP ? QUERY_NAME_MAP[uniqQuery] : '';
        const element = document.querySelector(uniqQuery);
    ...
    restore() {
        ...
        Entity.RESTORABLE_ATTRIBUTES.forEach(attr => {
            ...
            if (!location.search.includes('%2C')) {
                this.element.setAttribute(attr, value);
            }
        });
    }
}
Entity.RESTORABLE_ATTRIBUTES = ['stroke', 'transform', 'transform-origin'];
/**
...
class Service {
    constructor() {
        ...
        this.buttons = new Entity('.buttons');
        ...
        if (this.params.get('download') != null) {
            this.downloadImage();
        }
    }
    get bgColor() {
    ...
    get score() {
        ...
        const calculateScoreAverage = (values, min, max, isRotate = false) => {
    ...
    tweet() {
        ...
        const message = score < 6 ? 'キミは伝説の低級戦士……' :
...
class Control {
    ...
    animate(entity) {
        const update = ({ origin, x, y, scale, rotate }) => {
            ...
            rotate = (rotate + 9) % 360;
            if (entity.animation) {
                requestAnimationFrame(() => update({ origin, x, y, scale, rotate }));
            }
        };
