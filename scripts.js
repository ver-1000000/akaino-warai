...
class Service {
  ...
  constructor() {
    ...
    this.frame.element.classList.remove('loading');
    this.frame.element.style.setProperty('background-color', this.bgColor);
...
class Game {
  ...
  start() {
    this.service.frame.element.addEventListener('click', () => {
      ...
      if (this.currentEntity) {
        ...
        this.service.updateTopText();
      }
    });
  }

  animate(entity) {
    const update = ({ origin, x, y, hue, scale, rotate, stop })=> {
      ...
      rotate                 = (rotate + 9) % 360;
      entity.animationFrame  = requestAnimationFrame(() => update({ origin, x, y, hue, scale, rotate }));
    };
    update({ origin: random(10), x: random(128), y: random(128), hue: random(128), scale: random(5), rotate: random(360) });
  };
};
