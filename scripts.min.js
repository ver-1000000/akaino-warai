function _defineProperty(obj,key,value){if(key in obj){Object.defineProperty(obj,key,{value:value,enumerable:true,configurable:true,writable:true})}else{obj[key]=value}return obj}const random=(range=100)=>Math.round(Math.random()*range);const QUERY_NAME_MAP={".body":"\u8EAB\u4F53\u2015\u2015 Body",".nose":"\u9F3B\u2015\u2015 Nose",".fang .left":"\u5DE6\u306E\u30AD\u30D0\u2015\u2015 Left Fang",".fang .right":"\u53F3\u306E\u30AD\u30D0\u2015\u2015 Right Fang",".eye .left":"\u5DE6\u76EE\u2015\u2015 Left Eye",".eye .right":"\u53F3\u76EE\u2015\u2015 Right Eye"};/**
 * DOMを扱いやすくするためのラッパー。
 */class Entity{get searchParams(){const params=new URLSearchParams;Entity.RESTORABLE_ATTRIBUTES.map(x=>[x,this.element.getAttribute(x)]).forEach(([k,v])=>v&&params.append(`${this.uniqQuery}:${k}`,v));return params}constructor(uniqQuery){_defineProperty(this,"uniqQuery",void 0);_defineProperty(this,"name",void 0);_defineProperty(this,"element",void 0);_defineProperty(this,"animation",false);this.uniqQuery=uniqQuery;this.name=QUERY_NAME_MAP[uniqQuery]||"";this.element=document.querySelector(uniqQuery)}removeClass(...tokens){this.element.classList.remove(tokens)}addClass(...tokens){this.element.classList.add(tokens)}updateAttribute(key,value){if(value){this.element.setAttribute(key,value)}else{this.element.removeAttribute(key)}}reset(){Entity.RESTORABLE_ATTRIBUTES.forEach(attr=>this.updateAttribute(attr,null))}restore(){const params=Object.fromEntries(new URLSearchParams(location.search));Entity.RESTORABLE_ATTRIBUTES.forEach(attr=>{const value=params[`${this.uniqQuery}:${attr}`];if(value){this.updateAttribute(attr,value)}})}}/**
 * シングルトン的に利用するロジックをまとめたサービス。
 */_defineProperty(Entity,"RESTORABLE_ATTRIBUTES",["stroke","transform","transform-origin"]);class Service{get bgColor(){return`hsl(${this.hue}, 100%, 70%)`}get accentColor(){return`hsl(${(this.hue-90)%360}, 100%, 40%)`}get resultURL(){const params=new URLSearchParams;this.animEntities.forEach(x=>[...x.searchParams].forEach(([k,v])=>params.append(k,v)));params.append("hue",this.hue);return`https://ver-1000000.github.io/akaino-warai/?${params}`}constructor(){_defineProperty(this,"hue",random());_defineProperty(this,"world",new Entity(".world"));_defineProperty(this,"topText",new Entity(".top .text"));_defineProperty(this,"buttons",new Entity(".buttons"));_defineProperty(this,"saveImageButton",new Entity(".bottom .left.button"));_defineProperty(this,"tweetButton",new Entity(".bottom .center.button"));_defineProperty(this,"resetButton",new Entity(".bottom .right.button"));_defineProperty(this,"animEntities",Object.keys(QUERY_NAME_MAP).map(x=>new Entity(x)));_defineProperty(this,"params",new URLSearchParams(location.href));new Control(this);if(this.params.get("hue")!=null){this.restore()}if(this.params.get("download")!=null){this.downloadImage()}}downloadImage(){this.restore();this.buttons.addClass("hidden");svg2png(this.world.element).then(href=>{Object.assign(document.createElement("a"),{href,download:"akaino-warai.png"}).click();close()})}openDownloadWindow(){open(this.resultURL+"&download=")}tweet(){const url=encodeURIComponent(this.resultURL);const text=encodeURIComponent("\u3053\u3093\u306A\u6731\u732A\u306B\u306A\u3063\u3061\u3083\u3063\u305F\uFF01 \u6731\u732A\u308F\u3089\u3044\u3067\u6731\u732A\u3092\u5B8C\u6210\u3055\u305B\u3088\u3046\uFF01");const hashtag=encodeURIComponent("\u6731\u732A\u308F\u3089\u3044");const twitterUrl=`//twitter.com/intent/tweet?text=${text}&url=${url}&hashtags=${hashtag}`;open(twitterUrl,"_blank",`menubar=no,toolbar=no,resizable=yes,scrollbars=yes,width=710,height=400`)}reset(){this.hue=random(360);this.animEntities.forEach(x=>x.reset());new Control(this)}restore(){this.hue=this.params.get("hue");this.animEntities.forEach(x=>x.restore());this.world.element.style.setProperty("background-color",this.bgColor);this.finish();this.updateTopText()}finish(){this.world.addClass("loading");this.updateTopText("\u5B8C\u6210\uFF01\u2015\u2015 Finish!");this.buttons.removeClass("hidden")}updateTopText(text=""){this.topText.element.innerHTML=text}}/**
 * コントロールを記述するクラス。
